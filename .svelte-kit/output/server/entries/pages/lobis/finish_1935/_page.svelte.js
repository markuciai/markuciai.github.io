...
<div class="where-next too_early">${slots.default ? slots.default({}) : ``}<br><br>
${$language == "EN" ? `You need to fill <br>You need to go somewhere else.` : `${$language == "RU" ? `А вот пустое ведро <br>нужно наполнить водой.` : `${$language == "LA" ? `Nimis mane es! <br>Alicubi ire necesse est.` : `			    	Į tuščią kibirą <br>reikia pripilti vandens.`}`}`}</div>` : `${$visited == 8 ? `<img class="illustration"${add_attribute("src", task$6, 0)}>
<div class="where-next too_early">${slots.default ? slots.default({}) : ``}<br><br>
${$language == "EN" ? `You&#39;re too early! <br>You need to go somewhere else.` : `${$language == "RU" ? `Дивные сады и фруктовые деревья поместья <br>не растут, если их не поливать.` : `${$language == "LA" ? `Nimis mane es! <br>Alicubi ire necesse est.` : `			    	Jus atejote per anksti! <br>Pirma reikia rasti kelio pradžia.`}`}`}</div>` : `${$visited == 9 ? `<img class="illustration"${add_attribute("src", task$2, 0)}>
<div class="where-next too_early">${slots.default ? slots.default({}) : ``}<br><br>
...
<div class="where-next too_early">${slots.default ? slots.default({}) : ``}<br><br>
${$language == "EN" ? `You seem to have been everywhere, <br>except for the croquet court.` : `${$language == "RU" ? `Вы, кажется, уже везде были, <br>кроме площадки для крокета.` : `${$language == "LA" ? `Nimis mane es! <br>Alicubi ire necesse est.` : `			    	Išskyrus kroketo aikštelę,  <br>jūs jau atrodo visur apsilankėte.`}`}`}</div>` : `${$visited == 12 ? `<img class="illustration"${add_attribute("src", task$a, 0)}>
<div class="where-next too_early">${slots.default ? slots.default({}) : ``}<br><br>
